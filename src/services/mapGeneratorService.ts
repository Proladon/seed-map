...
// 定義類型
type BiomeKey = number
type MapData = BiomeKey[][]
type BiomePercentage = {
  name: string
  percentage: string
  color: string
}

type BiomePercentages = Record<BiomeKey, BiomePercentage>
...
/**
 * 驗證種子碼是否有效（10位數字）
 * @param {string} seedInput - 輸入的種子碼
 * @returns {string|null} - 錯誤訊息，如果有效則返回null
 */
export function validateSeed(seedInput: string): string | null {
  if (!/^\d{10}$/.test(seedInput)) {
    return '種子碼必須是10位數字（將使用預設種子）'
  }
  return null
}
 ...
 */
export function createNoiseFunction(seedNum: number): ReturnType<typeof createNoise2D> {
